...
describe('savedObjectsClient/errorTypes', () => {
  ...
  describe('GenericProductNotSupportedError', () => {
    it('makes an error identifiable as an EsUnavailable error', () => {
      const error = SavedObjectsErrorHelpers.createGenericProductNotSupportedError();
      expect(SavedObjectsErrorHelpers.isEsUnavailableError(error)).toBe(true);
    });

    it('makes an error identifiable as an GenericProductNotSupportedError error', () => {
      const error = SavedObjectsErrorHelpers.createGenericProductNotSupportedError();
      expect(SavedObjectsErrorHelpers.isGenericProductNotSupportedError(error)).toBe(true);
    });
    ...
    it('decorates the error message with the client method called', () => {
      const error = SavedObjectsErrorHelpers.createGenericProductNotSupportedError('foo');
      expect(error.output.payload).toHaveProperty(
        'message',
        'x-elastic-product not present or not recognized: could not foo'
      );
    });

    it('decorates the error message with the saved object type and id', () => {
      const error = SavedObjectsErrorHelpers.createGenericProductNotSupportedError('foo', {
        type: 'bar',
        id: '123',
      });
      expect(error.output.payload).toHaveProperty(
        'message',
        'x-elastic-product not present or not recognized: could not foo saved object [bar/123]'
      );
    });

    describe('error.output', () => {
      it('prefixes Product not found with elasticsearch call made', () => {
        const error = SavedObjectsErrorHelpers.createGenericProductNotSupportedError('baz');
        expect(error.output.payload).toHaveProperty(
          'message',
          'x-elastic-product not present or not recognized: could not baz'
        );
      });

      it('specifies the saved object type and id', () => {
        const error = SavedObjectsErrorHelpers.createGenericProductNotSupportedError('baz', {
          type: 'bar',
          id: '123',
        });
        expect(error.output.payload).toHaveProperty(
          'message',
          'x-elastic-product not present or not recognized: could not baz saved object [bar/123]'
        );
      });
