...
describe('404s from proxies', () => {
  it('returns unavailable errors', async () => {
    ...
    //   ...testServerSettings,
    // });
    // using this to try and create a saved object.
    const root = kbnTestServer.createRootWithCorePlugins({ ...testServerSettings });
    await root.preboot();
    ...
    // customResponse = { body: { reason: 'proxy response!' }, statusCode: 200 };
    // index a document:
    const docIndexed = await repository.create('MyType', {
      textField: 'I exist',
      boolField: 'true',
      id: '123',
    });
    ...
    // }
    const response = await repository.get(docIndexed.type, docIndexed.id);
