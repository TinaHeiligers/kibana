...
describe('SavedObjectsRepository', () => {
  ...
  describe('#create', () => {
    beforeEach(() => {
      ...
      client.index.mockImplementation((params) =>
        elasticsearchClientMock.createSuccessTransportRequestPromise(
          {
            _id: params.id,
            ...mockVersionProps,
    ...
    describe('errors', () => {
      ...
      describe('unsupported response error', () => {
        beforeEach(() => {
          client.create.mockImplementation((params) => {
        ...
        it(`throws on create when a response from an unsupported product is received`, async () => {
          const options = { id: '123', overwrite: false };
          await expect(
            savedObjectsRepository.create(type, attributes, options)
          ).rejects.toThrowError(
            SavedObjectsErrorHelpers.createGenericProductNotSupportedError('create', {
              type,
              id: '123',
            })
          );
        ...
        it(`throws on index when a response from an unsupported product is received`, async () => {
          const options = { id: '123', overwrite: true };
          await expect(
            savedObjectsRepository.create(type, attributes, options)
          ).rejects.toThrowError(
            SavedObjectsErrorHelpers.createGenericProductNotSupportedError('index', {
              type,
              id: '123',
            })
          );
